...
describe('Get recicipient notification controller (E2E)', () => {
  ...
  it('should be able to get recipient notifications', async () => {
    ...
    const response = await server.get(`${URL}/from/${recipientId}`);

    expect(response.statusCode).toBe(HttpStatus.OK);
    expect(response.body).toHaveLength(1);
    expect(response.body).toEqual(
      expect.arrayContaining([expect.objectContaining({ recipientId })]),
    );
  });
  ...
  it('should be able to get all recipient notifications', async () => {
    ...
    const response1 = await server.get(`${URL}/from/${uuid1}`);

    const response2 = await server.get(`${URL}/from/${uuid2}`);

    const response3 = await server.get(`${URL}/from/${uuid3}`);

    expect(response1.statusCode).toBe(HttpStatus.OK);
    expect(response1.body).toHaveLength(1);
    expect(response1.body).toEqual(
      expect.arrayContaining([expect.objectContaining({ recipientId: uuid1 })]),
    );
    expect(response2.statusCode).toBe(HttpStatus.OK);
    expect(response2.body).toHaveLength(2);
    expect(response2.body).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ recipientId: uuid2 }),
        expect.objectContaining({ recipientId: uuid2 }),
      ]),
    );
